...
/**
 * @author nwagar
 * @author JavaScriptKit
 * 
 * Script adapted from JavaScriptKit
 * http://www.javascriptkit.com/javatutors/loadjavascriptcss.shtml
 */
//Use this variable to store what files are already loaded
var filesLoaded = '';

/**
 * Include a JS file on demand
 * @param {Object} file The full path to the script file, relative to the web root
 */
var includeJSFile = function (file)
{
	//Check if the file's already loaded
	if (filesLoaded.indexOf('[+' + file + '+]') == -1)
	{
		...
		//Store the file name so it can't be loaded twice
		filesLoaded += '[+' + file + '+]';
	}
}
/**
 * Include a CSS file on demand
 * @param {Object} file The full path to the script file, relative to the web root
 * @param {Object} media Optional - Include a media type, if necessary
 */
var includeCSSFile = function (file, media)
{
	//Check if the file's already loaded
	if (filesLoaded.indexOf('[-' + file + '-]') == -1)
	{
		...
		//Include a media type, if provided
		...
		//Append the new link tag to <head>
		document.getElementsByTagName("head")[0].appendChild(fileref);
...
$(document).ready(function () {
	if ($('#contentWrapper').height() == 670) {
		var lowPoint = 0;
		
		$('#contentWrapper .component').each(function(){
			var tmpLowPoint = +($(this).height()) + (+($(this).position().top));
			
			if (tmpLowPoint > lowPoint) {
				lowPoint = tmpLowPoint;
			}
		});
		
		$('#contentWrapper').height(lowPoint);
	}
});
...
$(document).ready(function () {
	$('#signUpLink').click(function () {
		$.ajax({
			type: 'post'
			,url: 'signup.php?action=signup'
			,data: {
				csrfLock: $('#csrfLock').val()
			}
			,complete: function(xhr, status){
				...
				switch(xhr.status) {
					case 200:
						...
						break;
					case 201:
						modal.html('You have been added to this club\'s prospective member mailing list.');
						break;
					case 1223:
					case 204:
						modal.html('You\'re already a member of this club.<br />(You may be a prospective or inactive member, contact a club officer for more information.)');
						break;
					case 400:
						modal.html('Unable to complete the request');
						break;
					case 401:
						modal.html('You must be logged in as an undergraduate student in order to sign up.');
						break;
					case 403:
						modal.html('You are not an undergraduate student.  Only undergraduate students can be prospective club members.');
						break;
					case 500:
						modal.html('Sorry, but the server may be experiencing some technical difficulties and couldn\'t add you as a prospective member.  If this problem persists, please contact SA at <a href="mailto:sao@rochester.edu">SAO@rochester.edu</a>.');
						break;
				}
				
				modal.addClass('modal_content').append($('<div></div>')
					.addClass('modal_button_wrapper')
					.append($('<div></div>')
						.addClass('modal_button_l')
						.append($('<div></div>')
							.addClass('modal_button_r')
							.append($('<input type="button" />')
								.val('Close')
								.addClass('modal_button')
								.attr('id', 'signUpModalClose')
							)
						)
					)
				).prepend($('<div></div>')
				...
				$(modal).modal({opacity : 60, overlayCss: {backgroundColor:"#000"}});
				
				$('#signUpModal .modal_close,#signUpModal .modal_button').click(function () {
					$.modal.close();
				});
			}
		});
